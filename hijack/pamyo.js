...
class UI {
  ...
  static displayBooks() {
    ...
    const data_pamyo = Store.getBooks()

    data_pamyo.forEach((book) => UI.addBookToList(book))
  }
...
// 3.1 Event: Display data_pamyo (페이지 초기 로드시 실행)
...
class Store {
  ...
  static getBooks() {
    let data_pamyo
    if (localStorage.getItem('data_pamyo') === null) {
      data_pamyo = []
    } else {
      data_pamyo = JSON.parse(localStorage.getItem('data_pamyo'))
    }
    return data_pamyo
  }
  ...
  static addBook(book) {
    const data_pamyo = Store.getBooks()
    data_pamyo.push(book)
    localStorage.setItem('data_pamyo', JSON.stringify(data_pamyo))
  }
  ...
  static removeBook(isbn) {
    ...
    data_pamyo.forEach((book, index) => {
      if (book.isbn === isbn) {
        data_pamyo.splice(index, 1)
      }
    })
